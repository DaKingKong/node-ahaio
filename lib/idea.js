...
module.exports = function(client) {
    ...
    /**
     * Adds a public comment to a idea
     * @param {string} ideaId - Numeric ID or key of the idea the comment should be created for
     * @param {object} comment
     * @param {string} comment.body - HTML string of message body
     * @param {string} comment.email - The email of the user posting the comment
     * @param callback
     */
    function addPublicComment(ideaId, comment, callback) {
        if (_.isEmpty(ideaId)) { return callback(new Error('Feature ID required')); }
        if (_.isEmpty(comment) || !_.isObject(comment)) { return callback(new Error('Comment parameter missing or invalid.')); }
        if (_.isEmpty(comment.body)) { return callback(new Error('Missing or invalid comment.user object.')); }
        if (_.isEmpty(comment.email)) { return callback(new Error('Missing or invalid comment.email string.')); }

        client.post(`${route}/${ideaId}/idea_comments`, comment, callback);
    }
     ...
     * @param callback
     */
    function update(ideaId, changes, callback) {
